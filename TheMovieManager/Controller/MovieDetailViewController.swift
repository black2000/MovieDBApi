...
class MovieDetailViewController: UIViewController {
    ...
    @IBAction func favoriteButtonTapped(_ sender: UIBarButtonItem) {
        TMDBClient.markFavourite(movieId: movie.id, favourite:!isFavorite  , completion: handleFavouriteResponse(success:error:))
    }
    
    
    func handleFavouriteResponse(success : Bool , error : Error?) {
        
        if success {
            if isFavorite {
                MovieModel.favorites =
